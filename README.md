...
- `GET/POST/PUT/DELETE /api/productos`: gestión de productos. Los `GET` (listar/ver) son públicos, sin necesidad de login, para permitir el catálogo público de la tienda; crear/editar/eliminar siguen requiriendo admin. El campo `imagen_url` guarda la URL de Cloudinary de la imagen del producto (la imagen se sube directo desde el navegador a Cloudinary; este backend nunca recibe ni procesa los bytes de la imagen, solo guarda el link).
